...
def calculate_BM25(query , query_id , result_folder_path):
    rank = 0;
    BM25_dict = {}
    terms_in_query = query.split()
    doc_list = []
...
def runBM25(queries_file,result_folder_path):
    try:
        ...
        for each_query in queries:
            ...
            if(len(query) > 1):
                ...
                calculate_BM25(query[1],query_id,result_folder_path)
        f.close()
    except Exception as e:
            print(traceback.format_exc())
            pass
...
# call main function
def main_program(queries_file , result_folder_path):
    fetch_common_words('../../Dataset/common_words.txt')
    runBM25(queries_file , result_folder_path)
