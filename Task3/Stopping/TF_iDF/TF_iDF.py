...
path_unigram_term_count = '../../../Dataset/Indexer_Files_with_stopping/unigram_terms.txt'

# Set as path of file containing df for unigrams.
path_unigram_df = '../../../Dataset/Indexer_Files_with_stopping/unigram_df.txt'

# Set as path of file containing df for unigrams.
path_unigram_tf = '../../../Dataset/Indexer_Files_with_stopping/unigram_tf.txt'

# Set as path of file containing inverted index for unigrams.
path_inverted_index = '../../../Dataset/Indexer_Files_with_stopping/inverted_index_unigram.txt'

# Set as path for file containing all search queries. 
queries_file = '../../../Dataset/processed_queries_stopped.txt'

# Set as path for directory containing the corpus. 
corpus = '../../../Dataset/cleaned_files_with_stopping'
...
def tf_idf(query,query_id):
    ...
    file = open("TF_iDF_Ranking_with_stopping.txt",'a')
    file.write(str("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+"\n"))
    file.write("Query : "+str(queryStr)+"\n \n")
    for key,value in ranked_data:
        rank+=1;
        temp_str = str(query_id) + " " + "Q0" + " " + " " + str(key) + " " + str(rank) + " " + str(value) + " " + "TF_iDF_Stopping" + "\n"
        file.write(temp_str +"\n")
    file.write(str("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+"\n"))
    file.close()
